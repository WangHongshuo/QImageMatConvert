...
cv::Mat QImage2Mat_with_pointer(QImage &src)
{
    ...
    case QImage::Format_ARGB32_Premultiplied:
        dst = cv::Mat(src.height(), src.width(), CV_8UC4,
                      const_cast<unsigned char*>(src.constBits()), static_cast<size_t>(src.bytesPerLine()));
        break;
    // 3 channels
    case QImage::Format_RGB888:
        dst = cv::Mat(src.height(), src.width(), CV_8UC3,
                      const_cast<unsigned char*>(src.constBits()), static_cast<size_t>(src.bytesPerLine()));
        // warning: pay attention to this:
    ...
    case QImage::Format_Indexed8:
        dst = cv::Mat(src.height(), src.width(), CV_8UC1,
                      const_cast<unsigned char*>(src.constBits()), static_cast<size_t>(src.bytesPerLine()));
        break;
    default:
